...
main() {
    ...
    if [ "$STARTLOOP" -gt 0 ]; then
        ...
        if ! measure-display.sh --pattern-source=disp-tester --wfile=enable --rfile=enable --gfile=enable --bfile=enable --loop="$STARTLOOP" --measurement-delay=1 --quiet=yes --temp=ds18b20 > "$output_dir/start-gamut.csv"; then
    ...
    if ! measure-display.sh --pattern-source=disp-tester --wfile=enable --loop="$WHITESHIFTLOOP" --measurement-delay=1 --quiet=yes --temp=ds18b20 --interval="$WHITESHIFTINTERVAL" > "$output_dir/whiteshift.csv"; then
    ...
    if [ "$ENDLOOP" -gt 0 ]; then
        ...
        if ! measure-display.sh --pattern-source=disp-tester --wfile=enable --rfile=enable --gfile=enable --bfile=enable --loop="$ENDLOOP" --measurement-delay=1 --quiet=yes --temp=ds18b20 > "$output_dir/end-gamut.csv"; then
