...
/*  Himax Android Driver Sample Code for hx83181 chipset
 *
...
bool hx83181_init(void);


static void hx83181_mcu_burst_mode_enable(void)
{
...
static void hx83181_mcu_burst_mode_disable(void)
{
	uint8_t tmp_data[DATA_LEN_4];
	int ret;
...
static void hx83181_slave_AHB_reg_broadcast_write(uint8_t host_id, uint32_t waddr, uint8_t *wdata, uint8_t wlen)
{
	...
	if (wlen > MAX_I2C_TRANS_SZ) {
		...
		himax_mcu_register_write(0x10007418U, DATA_LEN_4,
			tmp_data);

		tmp_data[3] = (uint8_t)((waddr >> 24U) & 0xFFU);
		tmp_data[2] = (uint8_t)((waddr >> 16U) & 0xFFU);
		tmp_data[1] = (uint8_t)((waddr >> 8U) & 0xFFU);
		tmp_data[0] = (uint8_t)(waddr & 0xFFU);
		/* Write Access Address to*/
		himax_mcu_register_write(0x1000741CU, DATA_LEN_4, tmp_data);
		/* Write Data to*/
		himax_mcu_register_write(0x10007420U, DATA_LEN_4, wdata);
		/* Write command to*/
		tmp_data[3] = 0x22U;
		tmp_data[2] = 0x22U;
		tmp_data[1] = 0x22U;
		tmp_data[0] = 0x22U;
		himax_mcu_register_write(0x10007414U, DATA_LEN_4, tmp_data);
		...
		himax_mcu_register_write(addr_ctrl_fw, DATA_LEN_4,
				 tmp_data);
		do {
			himax_mcu_register_read(0x10007414U, DATA_LEN_4, tmp_data);
			I("%s: Check *(0x10007414U) => 0x%02X\n", __func__, tmp_data[0]);
			if ((tmp_data[0] == 0xAAU) && (tmp_data[1] == 0xAAU)) {
				I("%s: DONE\n", __func__);
				tmp_data[3] = 0x00U;
				tmp_data[2] = 0x00U;
				tmp_data[1] = 0x00U;
...
static void hx83181_slave_AHB_reg_broadcast_read(uint8_t host_id, uint32_t raddr, uint8_t *rdata, uint8_t rlen)
{
	uint8_t tmp_data[DATA_LEN_4] = { 0 };
	uint8_t cMax = 50U;
	uint8_t cnt = 0U;
	...
	if (rlen > MAX_I2C_TRANS_SZ) {
		...
		tmp_data[3] = (uint8_t)((raddr >> 24U) & 0xFFU);
		tmp_data[2] = (uint8_t)((raddr >> 16U) & 0xFFU);
		tmp_data[1] = (uint8_t)((raddr >> 8U) & 0xFFU);
		tmp_data[0] = (uint8_t)(raddr & 0xFFU);
		/* Write Access Address to*/
		himax_mcu_register_write(0x1000741CU, DATA_LEN_4, tmp_data);
		...
		do {
			...
			if ((tmp_data[0] == 0xAAU) && (tmp_data[1] == 0xAAU)) {
				I("%s: DONE\n", __func__);
				himax_mcu_register_read(0x10007420U, DATA_LEN_4, rdata);
				tmp_data[3] = 0x00U;
				tmp_data[2] = 0x00U;
				tmp_data[1] = 0x00U;
				tmp_data[0] = 0x00U;
...
static void hx83181_sense_on(void)
{
	uint8_t tmp_data[DATA_LEN_4];
	uint8_t retry = 0;
	int ret = 0;
...
static void hx83181_sense_off(void)
{
	uint8_t cnt = 0U;
	uint8_t tmp_data[DATA_LEN_4] = { 0 };
	uint8_t cMax = 14U;
...
static void hx83181_mcu_flash_dump_func(uint32_t start_addr,
...
static bool hx83181_mcu_flash_programming(const u8 *FW_content, unsigned int start_addr,
					  unsigned int length)
{
	...
	hx83181_mcu_burst_mode_disable();
...
void hx83181_mcu_information_check(void)
{
...
void hx83181_mcu_touch_information(void)
{
	uint8_t data[DATA_LEN_8] = { 0 };
	...
	for (retry = 0; retry < 5U; retry++) {
		...
		for (addr = fw_setting_addr; addr < addr_fw_setting_end;
			...
			if (addr == addr_fw_define_chip_rx_tx_num) {
				...
				ic_data->HX_Y_RES = ((uint16_t)data[2] << 8U) | (uint16_t)data[3];
				ic_data->HX_X_RES = ((uint16_t)data[0] << 8U) | (uint16_t)data[1];
			} else if (addr == addr_fw_define_total_rx_tx_rxic_num) {
	...
	private_ts->slave_ic_num = (ic_data->HX_RX_IC_NUM * ic_data->HX_TX_IC_NUM) - 1U;
...
bool hx83181_mcu_get_DSRAM_data(uint8_t *tmp_rawdata)
{
	unsigned int i = 0;
...
static void hx83181_func_re_init(void)
{
	g_core_fp.fp_sense_on = hx83181_sense_on;
	g_core_fp.fp_sense_off = hx83181_sense_off;
	g_core_fp.fp_flash_dump_func = hx83181_mcu_flash_dump_func;
	g_core_fp.fp_flash_programming = hx83181_mcu_flash_programming;
	g_core_fp.fp_check_CRC = himax_mcu_check_CRC;
	g_core_fp.fp_calculateChecksum = himax_mcu_calculateChecksum;
	g_core_fp.fp_touch_information = hx83181_mcu_touch_information;
	g_core_fp.fp_get_DSRAM_data = hx83181_mcu_get_DSRAM_data;
	g_core_fp.fp_burst_mode_enable = hx83181_mcu_burst_mode_enable;
	g_core_fp.fp_fts_ctpm_fw_upgrade = himax_mcu_fts_ctpm_fw_upgrade;
	g_core_fp.fp_slave_AHB_reg_broadcast_write = hx83181_slave_AHB_reg_broadcast_write;
	g_core_fp.fp_slave_AHB_reg_broadcast_read = hx83181_slave_AHB_reg_broadcast_read;

}

static bool hx83181_chip_detect(void)
{
	uint8_t tmp_data[DATA_LEN_4] = { 0 };
	...
	for (i = 0; i < 5; i++) {
		...
		if ((tmp_data[3] == 0x83U) && (tmp_data[2] == 0x18U) &&
			...
			if ((dd_version[1] == 0x01U && dd_version[0] == 0x00U)
				&& (dmy_config[1] == 0X21U && dmy_config[0] == 0x6EU)) {
				I("%s: found!\n");
			} else if ((dd_version[1] == 0x02U && dd_version[0] == 0x00U)
				&& (dmy_config[1] == 0X21U && dmy_config[0] == 0x6EU)) {
				I("%s: found!\n");
			} else if ((dd_version[1] == 0x03U && dd_version[0] == 0x001)
				&& (dmy_config[1] == 0X1AU && dmy_config[0] == 0x01U)) {
				I("%s: found!\n");
			} else if ((dd_version[1] == 0x04U && dd_version[0] == 0x001)
				&& (dmy_config[1] == 0X1AU && dmy_config[0] == 0x02U)) {
				I("%s: found!\n");
			} else {
				W("%s: @Unknown Type\n");
			}

			strscpy(private_ts->chip_name, HX_83181A_PWON, 30);
