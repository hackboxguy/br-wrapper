...
static void hx83194_mcu_burst_mode_enable(void)
{
...
static void hx83194_mcu_flash_dump_func(uint32_t start_addr,
...
static bool hx83194_mcu_flash_programming(const u8 *FW_content, unsigned int start_addr,
					  unsigned int length)
{
...
static bool hx83194_chip_detect(void)
{
	...
	for (i = 0; i < 5U; i++) {
		...
		if (id_check != HIMAX_PRODUCT_TYPE) {
			ret_data = false;
			E("IC ID: Current [HX%06X] not match!\n", id_check);
		}
